...
11. [Industry Alignment Review & Refinements (v1.4)](#11-industry-alignment-review--refinements-v14)
12. [New Entities Added in v1.5](#12-new-entities-added-in-v15)
13. [v1.5 Change Log](#13-v15-change-log)
...
| `acquisition_source` | VARCHAR(50) | | `BRANCH` \| `DIGITAL` \| `CC` \| `RM` \| `CAMPAIGN` \| `THIRD_PARTY` — moved here from subtypes |
| `source_branch_id` | UUID | FK → BRANCH | Branch that originated/onboarded the party — moved here from subtypes |
| `reason_inactive` | VARCHAR(500) | | Reason for inactive/dormant/closed status — moved here from subtypes |
...
| `deceased_date` | DATE | | Replaces `customer_deceased_date`; only meaningful when `party_status = DECEASED` |
| `notes` | TEXT | | |

> **v1.5 note:** `customer_sourced_by`, `source_branch_sol`, and `reason_inactive` have been **removed** — these are now on `PARTY` supertype (`acquisition_source`, `source_branch_id`, `reason_inactive`) and apply to all party types.
...
> **v1.5 note:** `customer_sourced_by`, `source_branch_sol`, and `reason_inactive` have been **removed** — now on `PARTY` supertype. `group_code` has been **removed** — it is derivable via `group_id → CORPORATE_GROUP.group_code`.
...
| `relationship_type` | VARCHAR(50) | NOT NULL | `JOINT_HOLDER` \| `GUARANTOR` \| `CO_APPLICANT` \| `AUTHORIZED_REP` \| `DIRECTOR` \| `SHAREHOLDER` \| `GROUP_MEMBER` \| `POWER_OF_ATTORNEY` \| `REFERENCE_CONTACT` |
...
### 3.24 ACCOUNT_PARTY_ROLE *(new — recommended)*

Industry-standard role bridge for account ownership and signing mandates. This avoids overloading `PARTY_RELATIONSHIP` for product-level rights and supports robust joint-account governance.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `account_party_role_id` | UUID | PK | |
| `account_id` | UUID | FK → ACCOUNT NOT NULL | |
| `party_id` | UUID | FK → PARTY NOT NULL | |
| `role_type` | VARCHAR(40) | NOT NULL | `PRIMARY_HOLDER` \| `JOINT_HOLDER` \| `AUTHORIZED_SIGNATORY` \| `BENEFICIAL_OWNER` |
| `operation_rule` | VARCHAR(30) | | `SINGLE` \| `JOINT_ANY_ONE` \| `JOINT_ALL` |
| `ownership_pct` | NUMERIC(5,2) | | For ownership disclosure |
| `start_date` | DATE | NOT NULL | |
| `end_date` | DATE | | |
| `is_mandate_active` | BOOLEAN | NOT NULL DEFAULT TRUE | |
| `daily_txn_limit` | NUMERIC(18,2) | | Optional role-level transaction ceiling |
| `created_at` | TIMESTAMP | NOT NULL | |
| `updated_at` | TIMESTAMP | NOT NULL | |

*Recommended unique constraint: `(account_id, party_id, role_type, start_date)`.*

---

### 3.25 PARTY_CONSENT_PREFERENCE *(new — recommended)*

Purpose-and-channel level consent model aligned to modern privacy regimes (UAE PDPL, GDPR-style controls). Replace single wide-row consent fields over time.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `preference_id` | UUID | PK | |
| `party_id` | UUID | FK → PARTY NOT NULL | |
| `consent_purpose` | VARCHAR(50) | NOT NULL | `MARKETING` \| `PROFILING` \| `THIRD_PARTY_SHARING` \| `AECB_ENQUIRY` |
| `channel` | VARCHAR(20) | NOT NULL | `SMS` \| `EMAIL` \| `PHONE` \| `PUSH` |
| `consent_status` | VARCHAR(20) | NOT NULL | `OPT_IN` \| `OPT_OUT` \| `WITHDRAWN` |
| `lawful_basis` | VARCHAR(30) | | `CONSENT` \| `CONTRACT` \| `LEGAL_OBLIGATION` |
| `source_system` | VARCHAR(50) | | |
| `effective_from` | TIMESTAMP | NOT NULL | |
| `effective_to` | TIMESTAMP | | |
| `recorded_at` | TIMESTAMP | NOT NULL | |
| `recorded_by` | UUID | FK → PARTY | |

*Recommended unique constraint: `(party_id, consent_purpose, channel, effective_from)`.*

---

### 3.26 PARTY_CONSENT_EVENT *(new — recommended)*

Immutable audit events for consent capture/change/withdrawal to satisfy evidentiary and supervisory requirements.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `consent_event_id` | UUID | PK | |
| `party_id` | UUID | FK → PARTY NOT NULL | |
| `preference_id` | UUID | FK → PARTY_CONSENT_PREFERENCE | Nullable for legacy-to-target migration |
| `event_type` | VARCHAR(30) | NOT NULL | `CAPTURED` \| `UPDATED` \| `WITHDRAWN` \| `EXPIRED` |
| `event_channel` | VARCHAR(20) | | `BRANCH` \| `DIGITAL` \| `CC` |
| `event_timestamp` | TIMESTAMP | NOT NULL | |
| `evidence_reference` | VARCHAR(500) | | Link to recording/signed form/workflow artifact |
| `event_by` | UUID | FK → PARTY | |
...
> **v1.5 note:** `sol_id` has been **removed** — derivable via `branch_id → BRANCH.sol_id`. The four `freeze_code_reason_N` columns have been **normalised** into the new `ACCOUNT_FREEZE_REASON` child entity (§12.1), which also captures freeze commencement date and releasing authority.
...
> **v1.5 note:** `salary_month` has been **removed** — derivable as `TO_CHAR(salary_date, 'YYYY-MM')`.
...
> **v1.5 note:** `preferred_language_code` has been **removed** — already captured on `PARTY.preferred_language`.
...
| `vip_tier` | VARCHAR(10) | | `NORMAL` \| `HIGH` \| NULL — replaces the two `is_vip_*` boolean flags |
| `is_staff_account` | BOOLEAN | NOT NULL DEFAULT FALSE | |
...
| `supplementary_party_id` | UUID | FK → PARTY | Preferred — links to the supplementary holder's CIF |
| `supplementary_cif_number` | VARCHAR(20) | | *Legacy/display field — derivable via `supplementary_party_id → PARTY.cif_number`; retain only for non-CIF holders* |
| `supplementary_customer_name` | VARCHAR(300) | | *Legacy/display field — derivable via `supplementary_party_id`; retain only for non-CIF holders* |
...
| `campaign_id` | UUID | FK → CAMPAIGN | Replaces VARCHAR campaign_id |
...
| `input_json` | JSONB | NOT NULL | Inputs (principal, tenure, rate etc.) |
...
| PARTY → ACCOUNT_PARTY_ROLE | 1:M | Recommended for product-level roles |
| ACCOUNT → ACCOUNT_PARTY_ROLE | 1:M | Joint holders/signatories/beneficial owners |
| PARTY → PARTY_CONSENT_PREFERENCE | 1:M | Purpose + channel consent state |
| PARTY_CONSENT_PREFERENCE → PARTY_CONSENT_EVENT | 1:M | Immutable consent audit trail |
...
## 11. Industry Alignment Review & Refinements (v1.4)

### 11.1 Executive Assessment

The v1.3 model is already strong on core CRM-banking constructs (party supertype, card supertype, Islamic finance separation, and portfolio denormalisation). The v1.4 refinement aligns it more closely with industry implementation patterns used in regulated banking data platforms.

---

### 11.2 Refinements Applied in This Version

1. **Resolved model inconsistencies in the Entity Overview**
   - Removed undefined entities from the overview map (`ACCOUNT_LIMIT`, `LOAN_STATEMENT`, `SME_REPAYMENT_SCHEDULE`, `CREDIT_LIMIT_DOCUMENT`, `CREDIT_CARD_STANDING_INSTRUCTION`) to keep the logical model internally consistent.

2. **Added product-level party role modelling**
   - Introduced `ACCOUNT_PARTY_ROLE` to model legal ownership/signatory mandates directly against `ACCOUNT`.
   - This is the industry-standard pattern for multi-holder and mandate controls; self-referential party links remain useful for social/corporate relationships but are not sufficient for account-operability rights.

3. **Upgraded consent architecture to purpose+channel+time granularity**
   - Added `PARTY_CONSENT_PREFERENCE` and `PARTY_CONSENT_EVENT` to support explicit consent lifecycle traceability and legal-basis metadata.
   - Existing `PARTY_CONSENT` can remain during migration as a compatibility snapshot.

4. **Corrected terminology**
   - Updated calculator input wording from “principle” to “principal”.

---

### 11.3 Recommended Deprecations / Rationalisation

1. **Deprecate `PARTY_REFERENCE` subtype in target state**
   - Prefer modelling reference contacts as ordinary `PARTY` rows with `PARTY_RELATIONSHIP.relationship_type = REFERENCE_CONTACT`.
   - Benefits: less subtype proliferation, cleaner onboarding APIs, and reusable role modelling.

2. **Constrain or phase out generic polymorphic FK usage in physical design**
   - For high-control entities (`DOCUMENT`, `MEMO`), consider explicit bridge tables in the physical model (e.g. `DOCUMENT_PARTY`, `DOCUMENT_ACCOUNT`, `DOCUMENT_LOAN`) for stronger referential integrity and easier lineage.

---

### 11.4 Additional Industry-Standard Recommendations (next iteration)

1. **Reference Data Standardisation**
   - Replace scattered status/category `VARCHAR` values with reference tables (`REF_CODESET`, `REF_CODE_VALUE`) and enforce FK constraints.

2. **Temporal / History Strategy**
   - Introduce effective-dated history entities for mutable compliance attributes (KYC, FATCA/CRS classification, customer risk profile), following SCD Type-2 semantics.

3. **Audit Column Consistency**
   - Enforce a common metadata contract (`created_at`, `updated_at`, `created_by`, `updated_by`, `record_source`, `as_of_timestamp`) across all mutable entities.

4. **Data Protection Controls**
   - Add field-classification metadata and encryption/tokenisation policy registry for PII and sensitive financial attributes; retain PCI-DSS posture for cards and extend to national identifiers.

5. **Event Lineage for BCBS 239 readiness**
   - Add lineage fields (`source_system`, `source_event_id`, `ingestion_batch_id`) on replicated core-banking entities to strengthen traceability and reconciliations.

---

## 12. New Entities Added in v1.5

### 12.1 ACCOUNT_FREEZE_REASON

Normalised child of `ACCOUNT`. Replaces the four `freeze_code_reason_N` columns, supporting an unlimited number of concurrent freeze reasons with full audit metadata.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `freeze_reason_id` | UUID | PK | |
| `account_id` | UUID | FK → ACCOUNT NOT NULL | |
| `freeze_code` | VARCHAR(30) | NOT NULL | Finacle freeze code |
| `freeze_description` | VARCHAR(200) | | Human-readable reason |
| `freeze_source` | VARCHAR(50) | | `BRANCH` \| `CC` \| `LEGAL` \| `SYSTEM` \| `COMPLIANCE` |
| `freeze_reference` | VARCHAR(100) | | Court order number / regulatory ref |
| `freeze_date` | DATE | NOT NULL | |
| `release_date` | DATE | | |
| `released_by` | UUID | FK → PARTY | |
| `status` | VARCHAR(20) | NOT NULL DEFAULT 'ACTIVE' | `ACTIVE` \| `RELEASED` |
| `created_by` | UUID | FK → PARTY | |
| `created_at` | TIMESTAMP | NOT NULL | |

*Composite unique constraint: `(account_id, freeze_code)` where `status = 'ACTIVE'`.*

---

### 12.2 CAMPAIGN

First-class campaign entity referenced by `LEAD.campaign_id`. Replaces the loose `VARCHAR(50)` campaign identifier.
...
| `campaign_type` | VARCHAR(30) | NOT NULL | `OUTBOUND` \| `INBOUND` \| `DIGITAL` \| `BRANCH` \| `EVENT` |
| `target_segment_id` | UUID | FK → SEGMENT | |
| `target_product_id` | UUID | FK → PRODUCT | |
| `start_date` | DATE | NOT NULL | |
| `end_date` | DATE | | |
| `status` | VARCHAR(20) | NOT NULL | `PLANNED` \| `ACTIVE` \| `PAUSED` \| `COMPLETED` \| `CANCELLED` |
| `owner_id` | UUID | FK → PARTY | Campaign owner (PARTY_EMPLOYEE) |
| `budget_aed` | NUMERIC(18,2) | | |
| `leads_generated` | INTEGER | | Materialised counter |
| `conversions` | INTEGER | | Materialised counter |
| `created_at` | TIMESTAMP | NOT NULL | |
| `updated_at` | TIMESTAMP | NOT NULL | |
...
Master collateral register. Provides a reusable collateral record that `CREDIT_LIMIT_COLLATERAL` and `RETAIL_LOAN_HOME_DETAILS` / `RETAIL_LOAN_AUTO_DETAILS` can reference instead of embedding collateral data redundantly.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `collateral_id` | UUID | PK | |
| `collateral_reference` | VARCHAR(50) | UNIQUE NOT NULL | Finacle collateral ref |
| `party_id` | UUID | FK → PARTY NOT NULL | Owner of the collateral |
| `collateral_type` | VARCHAR(50) | NOT NULL | `PROPERTY` \| `CASH` \| `SECURITY` \| `GUARANTEE` \| `VEHICLE` \| `GOLD` |
| `collateral_description` | VARCHAR(300) | | |
| `collateral_value` | NUMERIC(18,2) | | Market / forced-sale value |
| `collateral_currency` | CHAR(3) | | ISO 4217 |
| `valuation_date` | DATE | | |
| `linked_account_number` | VARCHAR(20) | | If cash / deposit collateral |
| `legal_charge_type` | VARCHAR(30) | | `FIRST_CHARGE` \| `SECOND_CHARGE` \| `LIEN` |
| `expiry_date` | DATE | | |
| `insurance_policy_id` | UUID | FK → INSURANCE_POLICY | If collateral is insured |
| `status` | VARCHAR(20) | NOT NULL | `ACTIVE` \| `RELEASED` \| `EXPIRED` |
| `created_at` | TIMESTAMP | NOT NULL | |
| `updated_at` | TIMESTAMP | NOT NULL | |

> **Update to `CREDIT_LIMIT_COLLATERAL`:** Add `collateral_id UUID FK → COLLATERAL` to link to the master register while retaining local override fields for backward compatibility.

---

### 12.4 BENEFICIARY_REGISTRY

Reusable beneficiary records saved by customers for outward transfers (SWIFT, local, AANI). Eliminates storing beneficiary details verbatim in every `TRANSFER_REMITTANCE` row.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `beneficiary_id` | UUID | PK | |
| `party_id` | UUID | FK → PARTY NOT NULL | Owning customer |
| `beneficiary_alias` | VARCHAR(100) | | Friendly name |
| `beneficiary_name` | VARCHAR(300) | NOT NULL | |
| `beneficiary_type` | VARCHAR(20) | NOT NULL | `OWN` \| `DOMESTIC` \| `INTERNATIONAL` |
| `account_number` | VARCHAR(34) | | Account number or IBAN |
| `bank_name` | VARCHAR(200) | | |
| `bank_swift` | VARCHAR(11) | | |
| `bank_country` | CHAR(3) | | ISO 3166-1 alpha-3 |
| `routing_code` | VARCHAR(30) | | IFSC / ABA / sort code |
| `currency` | CHAR(3) | | Preferred payment currency |
| `purpose_of_payment` | VARCHAR(200) | | Default POP for this beneficiary |
| `is_active` | BOOLEAN | NOT NULL DEFAULT TRUE | |
| `created_at` | TIMESTAMP | NOT NULL | |
| `updated_at` | TIMESTAMP | NOT NULL | |

> **Update to `TRANSFER_REMITTANCE`:** Add `beneficiary_id UUID FK → BENEFICIARY_REGISTRY` (nullable — not all transfers use saved beneficiaries).

---

### 12.5 ISLAMIC_FINANCE_REPAYMENT_SCHEDULE

Mirrors `RETAIL_LOAN_REPAYMENT_SCHEDULE` for Islamic financing facilities — required because Murabaha/Ijara schedules carry profit rather than interest components and are structured differently under AAOIFI standards.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `schedule_id` | UUID | PK | |
| `islamic_finance_id` | UUID | FK → ISLAMIC_FINANCE NOT NULL | |
| `installment_number` | INTEGER | NOT NULL | |
| `due_date` | DATE | NOT NULL | |
| `opening_principal` | NUMERIC(18,2) | | Remaining selling price |
| `installment_amount` | NUMERIC(18,2) | NOT NULL | |
| `principal_component` | NUMERIC(18,2) | | Cost price reduction |
| `profit_component` | NUMERIC(18,2) | | Profit (not interest) |
| `charges_component` | NUMERIC(18,2) | | |
| `balance_after_payment` | NUMERIC(18,2) | | |
| `actual_payment_date` | DATE | | |
| `status` | VARCHAR(20) | NOT NULL | `PENDING` \| `PAID` \| `OVERDUE` \| `WAIVED` |

*Composite unique constraint: `(islamic_finance_id, installment_number)`.*

---

### 12.6 REWARDS_TRANSACTION

Point-level transaction ledger for credit card rewards — allows auditable point earning and redemption history. Complements the `CREDIT_CARD_REWARDS` summary balance.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `rewards_txn_id` | UUID | PK | |
| `card_id` | UUID | FK → CREDIT_CARD NOT NULL | |
| `transaction_datetime` | TIMESTAMP | NOT NULL | |
| `transaction_type` | VARCHAR(30) | NOT NULL | `EARNED` \| `REDEEMED` \| `EXPIRED` \| `BONUS` \| `ADJUSTED` \| `REVERSED` |
| `points` | NUMERIC(14,2) | NOT NULL | Positive = earn; handled as positive in all rows — sign implied by `transaction_type` |
| `points_balance_after` | NUMERIC(14,2) | | Running total after this transaction |
| `reference_auth_transaction_id` | UUID | FK → CARD_AUTHORIZATION_TRANSACTION | Earning trigger |
| `partner_code` | VARCHAR(30) | | For partner earn/redeem (Skywards, etc.) |
| `description` | VARCHAR(300) | | |
| `created_at` | TIMESTAMP | NOT NULL | |

---

### 12.7 REF_CODESET and REF_CODE_VALUE

Centralized reference data framework to replace scattered hard-coded `VARCHAR` enumerations across the model. Aligned with ISO / SWIFT reference data governance patterns.
...
|--------|------|-------------|-------------|
| `codeset_id` | UUID | PK | |
| `codeset_code` | VARCHAR(50) | UNIQUE NOT NULL | e.g. `PARTY_STATUS`, `ACCOUNT_STATUS`, `LOAN_TYPE` |
| `codeset_name` | VARCHAR(200) | NOT NULL | |
| `domain` | VARCHAR(50) | | `PARTY` \| `PRODUCT` \| `CRM` \| `COMPLIANCE` |
| `is_extensible` | BOOLEAN | NOT NULL DEFAULT FALSE | Whether new values can be added at runtime |
| `is_active` | BOOLEAN | NOT NULL DEFAULT TRUE | |
...
| `codeset_id` | UUID | FK → REF_CODESET NOT NULL | |
| `code` | VARCHAR(50) | NOT NULL | e.g. `ACTIVE`, `DORMANT` |
| `display_name_en` | VARCHAR(200) | NOT NULL | |
| `display_name_ar` | VARCHAR(200) | | Arabic label |
| `sort_order` | INTEGER | | |
| `parent_code` | VARCHAR(50) | | For hierarchical code sets |
| `is_active` | BOOLEAN | NOT NULL DEFAULT TRUE | |

*Composite unique constraint: `(codeset_id, code)`.*

---

### 12.8 PARTY_ATTRIBUTE_HISTORY *(SCD Type-2 compliance audit)*

Captures effective-dated history for key mutable compliance attributes on a party: risk profile, KYC status, FATCA classification, and customer segment. Implements the SCD Type-2 pattern recommended in §11.4.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `history_id` | UUID | PK | |
| `party_id` | UUID | FK → PARTY NOT NULL | |
| `attribute_name` | VARCHAR(50) | NOT NULL | `RISK_PROFILE` \| `KYC_STATUS` \| `FATCA_STATUS` \| `CRS_CATEGORY` \| `SEGMENT` \| `PARTY_STATUS` |
...
| `source_system` | VARCHAR(50) | | |
| `changed_by` | UUID | FK → PARTY | |

*Index: `(party_id, attribute_name, effective_from DESC)` for current-value lookups.*

---

## 13. v1.5 Change Log

### 13.1 Redundancies Removed

| # | Entity | Column(s) Removed / Changed | Reason |
|---|--------|------------------------------|--------|
| 1 | `PARTY_INDIVIDUAL` | `customer_sourced_by`, `source_branch_sol`, `reason_inactive` | Duplicated in every subtype — promoted to `PARTY` supertype as `acquisition_source`, `source_branch_id`, `reason_inactive` |
| 2 | `PARTY_INDIVIDUAL` | `customer_deceased_date` | Renamed to `deceased_date` (redundant `customer_` prefix in context) |
| 3 | `PARTY_ORGANIZATION` | `customer_sourced_by`, `source_branch_sol`, `reason_inactive` | Same as #1 |
| 4 | `PARTY_ORGANIZATION` | `group_code` | Fully derivable via `group_id → CORPORATE_GROUP.group_code` |
| 5 | `ACCOUNT` | `sol_id` | Derivable via `branch_id → BRANCH.sol_id` |
| 6 | `ACCOUNT` | `freeze_code_reason_1/2/3/4` | Normalised into `ACCOUNT_FREEZE_REASON` child entity (§12.1) — eliminates the fixed-4-reason limit and adds audit metadata |
| 7 | `DEPOSIT` | `preferred_language_code` | Already on `PARTY.preferred_language` |
| 8 | `INSURANCE_POLICY` | `product_name` | Derivable via `product_id → PRODUCT.product_name` |
| 9 | `CREDIT_CARD` | `is_vip_normal`, `is_vip_high` | Mutually exclusive flags collapsed into single `vip_tier ENUM('NORMAL','HIGH')` |
| 10 | `PARTY_RELATIONSHIP` | `entity_type`, `cif_type` | `entity_type` is derivable via `related_party_id → PARTY.party_type`; `cif_type` had no defined semantics |
| 11 | `SALARY_TRANSACTION` | `salary_month` | Derivable as `TO_CHAR(salary_date, 'YYYY-MM')` |
| 12 | `LEAD` | `product_code` | Derivable via `product_id → PRODUCT.product_code` |
| 13 | `LEAD` | `campaign_id VARCHAR` | Promoted to `campaign_id UUID FK → CAMPAIGN` (§12.2) |

### 13.2 Columns Promoted to Supertype

| Column | From (subtypes) | To (supertype) |
|--------|-----------------|----------------|
| `acquisition_source` | `PARTY_INDIVIDUAL`, `PARTY_ORGANIZATION` | `PARTY` |
| `source_branch_id` | `source_branch_sol` in both subtypes | `PARTY` (FK → BRANCH) |
| `reason_inactive` | `PARTY_INDIVIDUAL`, `PARTY_ORGANIZATION` | `PARTY` |

### 13.3 New Entities Added

| # | Entity | Purpose |
|---|--------|---------|
| 12.1 | `ACCOUNT_FREEZE_REASON` | Normalises account freeze reasons into a proper child entity |
| 12.2 | `CAMPAIGN` | First-class campaign master, linked to `LEAD` |
| 12.3 | `COLLATERAL` | Central collateral register across lending products |
| 12.4 | `BENEFICIARY_REGISTRY` | Saved beneficiaries for transfers; eliminates repeated embedding in `TRANSFER_REMITTANCE` |
| 12.5 | `ISLAMIC_FINANCE_REPAYMENT_SCHEDULE` | AAOIFI-compliant repayment schedule with profit (not interest) terminology |
| 12.6 | `REWARDS_TRANSACTION` | Detailed point earn/redeem ledger complementing the `CREDIT_CARD_REWARDS` summary |
| 12.7 | `REF_CODESET` + `REF_CODE_VALUE` | Centralised reference data framework to govern status/category enumerations |
| 12.8 | `PARTY_ATTRIBUTE_HISTORY` | SCD Type-2 history for risk profile, KYC, FATCA, segment — satisfies §11.4 recommendation |

### 13.4 Relationship Additions

| Relationship | Cardinality | Notes |
|---|---|---|
| `ACCOUNT → ACCOUNT_FREEZE_REASON` | 1:M | Replaces 4x inline columns |
| `PARTY → CAMPAIGN` | M:M via `LEAD` | Campaign drives leads |
| `PARTY → COLLATERAL` | 1:M | Party-owned collateral register |
| `CREDIT_LIMIT → COLLATERAL` | M:M via `CREDIT_LIMIT_COLLATERAL` | Structured collateral link |
| `PARTY → BENEFICIARY_REGISTRY` | 1:M | Saved beneficiaries per customer |
| `TRANSFER_REMITTANCE → BENEFICIARY_REGISTRY` | M:1 | Optional saved-beneficiary link |
| `ISLAMIC_FINANCE → ISLAMIC_FINANCE_REPAYMENT_SCHEDULE` | 1:M | Financing repayment schedule |
| `CREDIT_CARD → REWARDS_TRANSACTION` | 1:M | Point event ledger |
| `PARTY → PARTY_ATTRIBUTE_HISTORY` | 1:M | SCD Type-2 compliance attributes |

### 13.5 Remaining Recommendations (Next Iteration)

1. **Migrate `PARTY_CONSENT` to `PARTY_CONSENT_PREFERENCE`** — the flat boolean consent entity (§3.8) should be formally deprecated once all consuming systems migrate to the purpose+channel preference model introduced in v1.4.
2. **`PARTY_REFERENCE` subtype deprecation** — pending migration to `PARTY_RELATIONSHIP.relationship_type = REFERENCE_CONTACT` as recommended in §11.3.
3. **Enforce `REF_CODE_VALUE` FKs** — progressively replace inline `VARCHAR` status/type columns with FK references to `REF_CODE_VALUE` to enforce value governance.
4. **`AUTHORIZED_REPRESENTATIVE` rationalisation** — for banked representatives use `ACCOUNT_PARTY_ROLE`; retain `AUTHORIZED_REPRESENTATIVE` only for non-CIF third parties with mandate documents.

---

*End of Data Model — RAKBANK CRM 360 Logical Model v1.5*
