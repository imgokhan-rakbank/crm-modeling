...
| `primary_rm_id` | UUID | FK → EMPLOYEE | Relationship Manager |
| `secondary_rm_id` | UUID | FK → EMPLOYEE | |
...
| `kyc_completed_by` | UUID | FK → EMPLOYEE | |
...
| `entity_type` | VARCHAR(30) | | `INDIVIDUAL` \| `CORPORATE` |
| `cif_type` | VARCHAR(30) | | |
...
| `account_manager` | UUID | FK → EMPLOYEE | |
| `created_at` | TIMESTAMP | NOT NULL | |
| `updated_at` | TIMESTAMP | NOT NULL | |
...
| `action_taken_by` | UUID | FK → EMPLOYEE | |
...
| `rm_id` | UUID | FK → EMPLOYEE | |
| `maker_id` | UUID | FK → EMPLOYEE | |
...
| `lead_assigned_to` | UUID | FK → EMPLOYEE | |
